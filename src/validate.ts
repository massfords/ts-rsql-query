...
import {SelectorConfig, SqlContext} from "./config";
import {isKnownOperator} from "./operators";
import {isAstNode, isComparisonNode} from "./ast";
import {parseISO} from "date-fns";

export const validate = (ast: ASTNode, context: SqlContext): { isValid: true } | { isValid: false, err: string } => {
    if (ast.operator == "and" || ast.operator == "or") {
        ...
        if (!isAstNode(lhs)) {
            return {isValid: false, err: `invalid operand type: ${JSON.stringify(lhs)}`};
        }
        ...
        if (result.isValid) {
            for (let i = 1; i < ast.operands.length && result.isValid; i++) {
                ...
                if (!isAstNode(rhs)) {
                    return {isValid: false, err: `invalid operand type: ${JSON.stringify(rhs)}`};
                }
        ...
        if (!isAstNode(operand)) {
            return {isValid: false, err: `invalid operand type: ${JSON.stringify(operand)}`};
        }
        return {isValid: true};
    } else if (isComparisonNode(ast)) {
        if (!isKnownOperator(ast.operator)) {
            return {isValid: false, err: `unknown operator: ${JSON.stringify(ast.operator)}`};
        }
        const value = ast.operands![0] ?? null;
        if (!value) {
            return {isValid: false, err: `missing value for selector: ${JSON.stringify(ast.selector)}`};
        }
        if (!("lax" in context)) {
            const selector = context.selectors[ast.selector];
            if (!selector) {
                return {isValid: false, err: `unknown selector: ${JSON.stringify(ast.selector)}`};
            }
            if (typeof selector === "object") {
                if (!isValueValid(selector, value)) {
                    if ("enum" in selector) {
                        return {isValid: false, err: `bad selector value for "${ast.selector}": "${value}" must be one of [${selector.enum}]`}
                    }
                    return {isValid: false, err: `bad selector value for "${ast.selector}": "${value}" is not a ${selector.type}`}
                }
            }
        }
        return {isValid: true};
