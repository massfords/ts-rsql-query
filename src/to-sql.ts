...
export const toSql = (input: ASTNode | string, context: SqlContext): {isValid: true, sql:string} | {isValid:false, err:string} => {
    ...
    if (ast.operator == "and" || ast.operator == "or") {
        ...
        switch (op) {
            case "==": {
                ...
                return `${selector}${leadingWildcard || trailingWildcard ? ` ilike ` : "="}$${values.length}`
            }
            case "!=": {
                invariant(ast.operands);
                invariant(ast.operands[0]);
